...
partnerImage.post(
  ...
  (req, res) => {
    try {
      ...
      const q = "INSERT INTO partner_Image (img) VALUES (?)";
      ...
      db.query(q, value, function (err, data) {
        ...
        return res
          .status(201)
          .json({ message: "Uploaded Successfully", img });
      });
    } catch (error) {
...
partnerImage.delete("/partnerimage/:id", (req, res) => {
    ...
    try {
      ...
      db.query(q, [id], function (err, data) {
        ...
        cloudinary.uploader.destroy(public_url, function (err, result) {
          ...
          const q =
            "UPDATE partner_Image SET img = ? WHERE id = ?";
